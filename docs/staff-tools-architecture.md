...
- `property_prospects`, `property_prospect_photos` — rent-to-rent deal-scouting board (any region), photos/videos in the private `property-scouting` Storage bucket. `property_prospects.feasibility_sheet_url` holds a per-property link to บอสตอง's own Google Sheet feasibility calc (shown as a "📊" open-link badge on the card and an "เปิด ↗" button in the modal) — not generated by this tool, just stored/linked. `property_prospects.key_money` is the one-time non-refundable ค่าเซ้ง/เงินกินเปล่า, deliberately a separate numeric field from `deposit` (the refundable security deposit) since they're financially very different — shown on the card only when set
